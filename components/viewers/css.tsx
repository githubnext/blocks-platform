...
export function CssViewer({ contents }: { contents: string }) {
  const { tree, flattenedRules, widelyApplicableAttributes } = useMemo(() => {
    const tree = toJSON(contents)
  ...
  return <div className="p-8" style={{
    background: (flattenedRules?.["body"]?.background || flattenedRules?.["body"]?.backgroundColor || "white"),
    color: (flattenedRules?.["body"]?.color || "black"),
  }}>
    {Object.keys(flattenedRules).map(selector => {
      return (
        <TreeItem selector={selector} attributes={flattenedRules[selector]} inheritedAttributes={widelyApplicableAttributes} />
      )
    })}
  </div>
}
...
const TreeItem = ({ selector = "", attributes = {}, inheritedAttributes = {} }: {
  ...
  return (
    <div className="py-3 flex">
      <div className="font-mono w-40 sticky top-2 pt-2 h-8 flex-none">{selector}</div>
      <div>
        {cssVariableNames.map(variable => {
          return (
            <div className="flex items-center text-xs m-1" key={variable}>
              <div className="font-mono flex-none w-60">
                {variable}
              </div>
      ...
      <ItemRenderer selector={selector} attributes={{ ...inheritedAttributes, ...attributes }} />
    </div>
  )
}
...
const Attribute = ({ value }: { value: string }) => {
  ...
  if (isColor) return (
    <div className="h-6 w-6 rounded" style={{
...
const ItemRenderer = ({ selector, attributes }: { selector: string, attributes: any }) => {
  ...
  if (selector === "p") return <p style={attributes}>This is what a paragraph looks like. Lorem ipsum dolor sit amet consectetur adipisicing elit. Inventore rem non consectetur.</p>
  return (
    // @ts-ignore
