...
export default function AnnotateReactViewer({ contents, meta }: ViewerProps) {
  ...
  const defaultAnnotations = annotations
    ? JSON.parse(decodeURIComponent(annotations as string))
    : [];

  const componentName = name.split("/").pop().split(".")[0];

  const onSubmit = async ({ annotations, title, description }) => {
    const url = `https://composable-github.vercel.app?owner=${owner}&repo=${repo}&path=${path}&viewerOverride=annotate-react&annotations=${encodeURIComponent(
      ...
      <div className="flex-1 p-6 pt-0 z-10">
        <div className="flex items-center pb-1">
          ...
          {annotationTypes.map(({ id, name }) => (
            <button
              key={id}
              className={`text-gray-800 text-xs uppercase tracking-widest py-2 px-5 rounded-full ${
                annotationType === id
      ...
      <div className="flex-1 p-6 self-stretch max-h-[40em] max-w-[35em]">
        {!!annotations.length && hasEdited && (
          ...
          >
            ...
            <input
              className="bg-white border border-gray-300 focus:outline-none focus:border-indigo-500 py-2 px-4 block w-full appearance-none leading-normal"
              type="text"
              placeholder="Title"
              value={issueTitle}
              onChange={(e) => {
            ...
            />
            <textarea
              className="bg-white border border-gray-300 flex-1 focus:outline-none focus:border-indigo-500 py-2 px-4 block w-full appearance-none leading-normal resize-none"
            ...
            />
            <button
              className={`bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white py-3 px-4 rounded-full transition ${
                issueTitle.length > 0 && issueDescription.length > 0
                  ? "opacity-100"
                  : "opacity-50"
              }`}
              type="submit"
