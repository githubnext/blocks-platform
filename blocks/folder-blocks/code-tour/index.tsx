...
const Step = ({
  ...
  return (
    <button
      className={`relative group w-full text-left py-3 px-3 ${
        isSelected ? "bg-[#0A69DA] text-white" : "bg-white"
      }`}
      onClick={onSelect}
    >
      ...
      <div className="flex flex-1 overflow-y-auto mt-1">
        ...
        >
          <Code
            code={content}
            language={language}
            highlightRangeStart={codeSnippet?.start_line}
            highlightRangeEnd={
              codeSnippet &&
              codeSnippet?.start_line + codeSnippet.code.split("\n").length - 1
            }
            onLineClick={
              isEditing
                ? (lineIndex) => {
  ...
  return (
    <div className="code text-sm w-full overflow-x-auto" ref={container}>
      <SyntaxHighlighter
        ...
        lineNumberStyle={{
          padding: "0.03em 0.6em 0.03em 0.6em",
          opacity: 0.45,
        }}
        wrapLines
        lineProps={(i) => {
