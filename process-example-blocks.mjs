...
// in process-example-blocks.mjs, run during the \`postinstall\` npm script\n\n`;
  ...
  indexContents += `\n`;
  indexContents += `export default ${JSON.stringify(
    blocksObject,
    null,
    2
  ).replace(/\"/g, "")}`;
...
function convertIdToVariableName(id) {
  return _.camelCase(id).replace(/^\d/, "");
}
  ...
  files.forEach(async (file) => {
    ...
    if (extension === "css") {
      ...
      const block = blocks.find((block) =>
        block.entry.includes(file.split("/").slice(1, 4).join("/"))
      );
      if (!block)
        console.log(
          "No block found for",
          file.split("/").slice(1, 4).join("/")
        );
...
async function copyExcalidrawAssetsToLocal() {
  const folderNames = ["excalidraw-assets", "excalidraw-assets-dev"];
